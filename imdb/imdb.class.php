<?php
 ...
 /* $Id: imdb.class.php 696 2014-08-05 16:57:23Z DJMcTom $ */
  ...
  * @version $Revision: 696 $ $Date: 2014-08-05 17:57:23 +0100 (Tue, 05 Aug 2014) $
  */
class imdb extends movie_base {
 ...
 #-----------------------------------------------------------[ Constructor ]---
  ...
  function __construct($id, mdb_config $config = null) {
    ...
    $this->revision = preg_replace('|^.*?(\d+).*$|','$1','$Revision: 696 $');
    $this->setid($id);
  }
...
/*          // Get bigsrc from linked photo page. (proposed by ticket:327 -- seems to result in the same as above, so keeping it just-in-case)
    ...
    if (empty($this->goofs)) {
      ...
      if ( @preg_match_all('@<h4 class="li_group">(.+?)(!?&nbsp;)</h4>\s*(.+?)\s*(<h4 class="li_group">|<div id="top_rhs_wrapper")@ims',$this->page["Goofs"],$matches) ) {
        ...
        for ($i=0;$i<$gc;++$i) {
          ...
          $ic = count($goofy[0]);
          for ($k=0;$k<$ic;++$k) $this->goofs[] = array("type"=>$matches[1][$i],"content"=>str_replace('href="/','href="https://'.$this->imdbsite.'/',$goofy[1][$k]));
        }
      }
    }
    return $this->goofs;
    ...
    if ( empty($this->moviequotes) ) {
      ...
      if (preg_match_all('!class="quote soda (odd|even)"\s*><p>\s*(.*?)\s*</p>\s*<div class=!ims',str_replace("\n"," ",$this->page["Quotes"]),$matches))
        foreach ($matches[2] as $match) {
          $this->moviequotes[] = "<p>".str_replace('href="/name/','href="https://'.$this->imdbsite.'/name/',preg_replace('!<span class="linksoda".+?</span>!ims','',$match))."</p>";
        }
    }
 ...
 protected function convertIMDBtoRealURL($url) {
   ...
   $req = new MDB_Request("https://".$this->imdbsite.$url, $this);
